...
init() {
        ...
        if (href && (href.startsWith("/") || href.startsWith("#"))) {
            ...
            if (href.startsWith("/")) {
                ...
                if (route) {
                    ...
                    const found = Object.values(this.routes).find(
                        (r) => r.section === href,
                    );
                    if (found) {
    ...
    document
        .querySelectorAll(".navbar__mobile a, .navbar__nav a")
        .forEach((link) => {
    ...
    // ===== اصلاح: مدیریت مسیر اولیه با بررسی وضعیت کاربر =====
    let initialPath = window.location.pathname || "/";

    // دریافت وضعیت کاربر از State
    const user = State.get("currentUser");

    // اگر مسیر لاگین یا ثبت نام است
    if (initialPath === "/login" || initialPath === "/register") {
        if (user) {
            // اگر کاربر لاگین است، به داشبورد هدایت شود
            this.navigate("dashboard", true);
            this.initialized = true;
            console.log("✅ Router مقداردهی شد (کاربر لاگین => هدایت به داشبورد).");
            return;
        } else {
            // اگر کاربر لاگین نیست، به خانه هدایت شود (تا صفحه اصلی نمایش داده شود)
            // این کار از نمایش صفحه لاگین در بارگذاری اولیه جلوگیری می‌کند
            initialPath = "/";
        }
    }

    // بارگذاری مسیر اولیه
    // اگر مسیر خالی یا فقط / است، به خانه بروید
    if (initialPath === "/" || initialPath === "") {
        this.navigate("home", true);
    } else {
        const route = this._findRouteByPath(initialPath);
        if (route) {
  ...
  _findRouteById(id) {
    ...
    let route = Object.values(this.routes).find(
      (route) => route.id === normalizedId,
    );
    if (!route) {
      route = Object.values(this.routes).find(
        (route) => route.component === normalizedId,
      );
    }
  ...
  _normalizePath(path) {
    ...
    if (cleanPath.includes("/index.html")) {
      cleanPath =
        cleanPath.substring(0, cleanPath.indexOf("/index.html")) || "/";
    }
